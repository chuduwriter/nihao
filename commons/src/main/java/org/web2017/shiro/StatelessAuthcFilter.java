...
public class StatelessAuthcFilter extends PermissionsAuthorizationFilter {
  ...
  public boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) throws IOException {
   ...
   int num=(int)servletPath.charAt(servletPath.length()-1);
    if(num>48 && num <57){
     servletPath=servletPath.substring(0,servletPath.length()-1)+"*"+":"+(((HttpServletRequest) request).getMethod());
    }
    else{
      servletPath=servletPath+":"+((HttpServletRequest) request).getMethod();
    }
